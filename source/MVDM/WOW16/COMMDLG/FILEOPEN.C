...
#if DPMICDROMCHECK
/* With IsCDROMDrive, these are no longer needed.  21 Jan 2000  clarkc */
WORD wCDROMIndex;
...
#else
BOOL IsCDROMDrive(WORD);              /* Added 21 Jan 2000  clarkc */
#endif
 ...
 * History:  14 March 2000  clarkc   Wrote it.
 *--------------------------------------------------------------------------*/
 ...
 * the filename.   Bug #5392.   10 March 2000   clarkc
 */
 ...
 * Bug #13463.    8 October 2000  Clark Cyr
 */
 ...
 * Bug #11214           4 August 2000   Clark Cyr
 */
  ...
  if (wWinVer >= 0x030A)
    ...
    {                   /* Windows 3.00, sysmodal bug.  clarkc   2 May 2000 */
  ...
  if (lpOfn->lpstrFile)
    {
      if (cb > (WORD)lpOfn->nMaxFile)
        ...
        /* Set nFileOffset to 1st file     10 Jun 2000   clarkc */
 ...
 * letter.   22 August 2000        Clark Cyr
 */
 ...
 * would have already switched to it.        26 July 2000     Clark Cyr
 */
 ...
 * caught by ParseFile.    18 July 2000     Clark Cyr
 ...
 * it, it's of 0 size.    3 September 2000    Clark Cyr
 */
              if (bUNCName)
          ...
          else            /* Unable to create it */
            {
               ...
               * filename is invalid.           29 July 2000  Clark Cyr
               */
              if ((of.nErrCode != OF_WRITEPROTECTION) &&
                  (of.nErrCode != OF_DISKFULL)        &&
                  (of.nErrCode != OF_ACCESSDENIED)    &&
...
#if STRIPBRACKETS
/* Strip brackets off.  14 Jan 2000  clarkc */
 ...
 * done to see how AnsiNext incremented the pointer.  6 Jun 2000  ClarkC
 */
      while (*lpch && (*lpch != ']'))
          lpch = AnsiNext(lpch);
      *lpch = '\0';
        ...
        switch (HIWORD(lParam))
          {
            ...
            case CBN_DROPDOWN:
              ...
              /* versions pre-Win 3.1.  10 March 2000   clarkc     */
            ...
            case CBN_CLOSEUP:
              ...
              /* done as needed.   10 March 2000   clarkc          */
 ...
 * Assumption no longer valid.  Thanks, UTIF.  18 Jan 2000  clarkc
 ...
 * no longer around, though the code is a bit fatter.  7 May 2000  clarkc
 */
 ...
 * place the null string after the space.    18 July 2000    ClarkC
 */
                  szTempField[0] = '\0';
                  iSel = cbCaption;
 ...
 *           28-Jan-2000 11:00:00 Clark R. Cyr  [clarkc]
 *           lpExtension does not need to be null terminated.
 *           30-Jan-2000          Clark R. Cyr  [clarkc]
 *--------------------------------------------------------------------------*/
...
BOOL IsCDROMDrive(WORD wDrive)   /* Added 21 Jan 2000  clarkc */
{
  if (bWLO)               /* if we are under WLO */
      return(FALSE);      /* 19 Feb 2000  clarkc */
 ...
 *           than the buffer we've allocated.  Clark R. Cyr  20 August 2000
 *
 *  return   TRUE/FALSE
 *-------------------------------------------------------------------------*/
...
BOOL IsRamDrive(WORD wDrive)
{
  ...
  if (bWLO)               /* if we are under WLO */
      return(FALSE);      /* 19 Jan 2000  clarkc */
