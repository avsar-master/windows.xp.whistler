...
#include <Memory.h>				// For Macixposh memory calls
#include <Files.h>				// Macixposh file i/o calls
...
MYDBG(Macixposh(GLOBAL)) HMEM MYDBG(NotMacixposh(GLOBAL))
...
{
	Macixposh(#pragma unused (fForce))
	...
	if (hmemrgmemBlock == (HMEM)NULL)
		...
		Macixposh(hmemrgmemBlock = (HMEM)NewHandleClear(
			cBlockInc * sizeof(struct MEMBLOCK)));
		...
		Macixposh(SetHandleSize((Handle)hmemrgmemBlock,
			GetHandleSize((Handle)hmemrgmemBlock) +
			cBlockInc * sizeof(struct MEMBLOCK)));
		Macixposh(if (MemError() != noErr)
...
// Note (non-Mac): We allocate a 4-byte minimum block, so our size might not
// always exactly match the application's
MYDBG(Macixposh(GLOBAL)) BOOL MYDBG(NotMacixposh(GLOBAL))
	FWizMemReAlloc(HMEM hMem, unsigned long cbMem, BOOL fForce,
	BOOL fClear)
{
	Macixposh(#pragma unused (fForce))

	NotMacixposh(HMEM hMemNew;)
	Macixposh(unsigned long cbMemCurMac;)
#if DBG
	...
	FWizMemFree(HMEM hMem)
{
	...
	Macixposh(DisposHandle((Handle)hMem));
	Macixposh(fOk = (MemError() == noErr));
	...
	if (cmemBlockMac == 0)
		...
		Macixposh(DisposHandle((Handle)hmemrgmemBlock));
		Macixposh(Assert(MemError() == noErr));
...
CHAR FAR *FPWizMemLock(HMEM hMem)
{
	...
	if (hMem != hmemrgmemBlock)
		...
		Macixposh(fpMemBlock = (struct MEMBLOCK *)*((Handle)hmemrgmemBlock));
		// Dos(Lock)
		// iBlock set earlier
...
BOOL FWizFileExist(LPSPATH lpFullPath)
{
	...
	CHAR *psz = szTmp;
#endif //DOS
	NotMacixposh(CHAR FAR *szFullPath = lpFullPath;)
...
#endif //DBG
	NotMacixposh(CHAR FAR *szFullPath = lpFullPath;)
	Macixposh(CHAR FAR *szFullPath = lpFullPath->lpszFilePath;)
	Macixposh(FInfo finfo;)
	Macixposh(OSErr oserr;)
	Macixposh(StringPtr spFullPath;)
	unsigned long cbCache;
	HMEM hmemCache;
	BOOL fRealCache = fCache;		// Real or fake cache (0 byte)?
	NotMacixposh(HMEM hmemFilename;)
...
#endif //WIN32
		Macixposh(flags = fsWrPerm);
		NotMacixposh(fRealCache = fFalse);	// Only support caching for read-only
		break;
	case wTypeReadWrite:
		Dos(flags |= O_RDWR);
#ifdef WIN32
...
LSwitchErr:
...
#endif //WIN32
	Macixposh(fpFileBlock->volRefNum = lpFullPath->volRefNum);
	Macixposh(fpFileBlock->dirID = lpFullPath->dirID);
	...
	if (fCache)
		...
		NotMacixposh(CHAR FAR *fpFullPath;)
...
LDoneCache:
		...
		AssertDoSz(fpFileBlock = (struct FILEBLOCK FAR *)FPWizMemLock(hFile),
			"FPWizMemLock(hFile) failed.  Fatal.");
		fpFileBlock->fCache = NotMacixposh(fTrue) Macixposh(fRealCache);
		fpFileBlock->fOpen = NotMacixposh(fFalse) Macixposh(fTrue);
		fpFileBlock->hFile = NotMacixposh(-1) Macixposh(hPhysFile);
		...
		NotMacixposh(fpFileBlock->hmemFilename = hmemFilename);
...
BOOL FWizFileClose(HFILE hFile)
{
	...
	if (!fpFileBlock->fCache || fpFileBlock->fOpen)
		...
		Macixposh(if (FSClose(fpFileBlock->hFile) == noErr))
#endif /* WIN32 */
			fClosed = fTrue;
		else
			{
	...
	if (fpFileBlock->fCache)
		{
		NotMacixposh(AssertDo(FWizMemFree(fpFileBlock->hmemFilename)));
	...
	unsigned long ibSeek, CHAR FAR *rgbBuffer)
{
	...
	Macixposh(OSErr oserr;)			// Return from Mac calls
	Macixposh(long cbReadMac;)		// Mac wants a long *
	CHAR FAR *pbBuffer = rgbBuffer;		// Where to read into
	NotMacixposh(BOOL fError = fTrue;)	// To save code...
#if DBG
	BOOL fFound;
#endif //DBG
	...
	if (fpFileBlock->fCache)
		{
		NotMacixposh(AssertSz(!fpFileBlock->fOpen,
	...
	unsigned long ibSeek, CHAR FAR *rgbBuffer)
{
	...
	BOOL fhFileLock = fFalse;		// Is the hFile locked?
	Macixposh(OSErr oserr;)			// Return from Mac calls
	Macixposh(long cbWriteMac;)		// Mac wants a long *
#if DBG
	BOOL fFound;
#endif //DBG

	NotMacixposh(Assert(cbMaxPhysFileBuffer <= 65535));	// We depend on this
...
BOOL FWizFileSeek(HFILE hFile, unsigned long ibSeek)
{
...
#else /* WIN32 */
	NotMacixposh(AssertSz(fpFileBlock->ibFile ==
	...
	if ((fpFileBlock->ibFile == ibSeek) && (!fpFileBlock->fCache
		Macixposh(|| (fpFileBlock->cbCache == fpFileBlock->cbFile))))
		goto LOk;					// Null seek
...
#else /* WIN32 */
	NotMacixposh(ibPhys = lseek(hPhysFile, ibSeek, 0 /* SEEK_SET */));
	Macixposh(oserr = SetFPos(hPhysFile, 1 /* fsFromStart */, ibSeek));
#endif /* WIN32 */
...
unsigned long IbWizFileGetEOF(HFILE hFile)
{
	...
	NotMacixposh(AssertSz(fpFileBlock->ibFile ==
		...
		"fpFileBlock->ibFile != tell() - bogus file, or ibFile bad!"));
#endif /* WIN32 */
	Macixposh(oserr = GetFPos(hPhysFile, (long *)&ibEOF));
	Macixposh(AssertSz((oserr == noErr) &&
...
BOOL FWizFileTruncate(HFILE hFile, unsigned long cbLen)
{
	...
	Macixposh(OSErr oserr;)				// Return from Mac calls
#if DBG
	Macixposh(unsigned long ibEOF;)		// File size (EOF position)
	BOOL fFound;
#endif //DBG
