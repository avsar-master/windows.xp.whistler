	...
	WORD cbRead, BOOL fAdjustBegin)
{
	...
	NotMacixposh(Assert(*(lpchT - 1) == CR));
...
BOOL FUpdateUdrInfoAddToFile(UdrInfo FAR *lpUdrInfo, BOOL fForce)
{
	...
	NotMacixposh(lpSPath = lpSzPath);
	Macixposh(spathT.lpszFilePath = lpSzPath);
	Macixposh(spathT.volRefNum = lpUdrInfo->volRefNum);
	Macixposh(spathT.dirID = lpUdrInfo->dirID);
	Macixposh(lpSPath = &spathT);
...
WORD UdrIdentifyScr(SCR FAR *lpScr, WORD FAR *lpScrLen, WORD udrpropType)
{
	...
	*lpScrLen = ((lpScrT - lpScr) NotMacixposh(+ 1));
...
BOOL FUdrInfoFileWrite(UdrInfo FAR *lpUdrInfo)
{
	...
	Macixposh(lpspathUdr = (LPSPATH)&spathUdr;)
	Macixposh(lpspathUdr->volRefNum = lpUdrInfo->volRefNum;)
	Macixposh(lpspathUdr->dirID = lpUdrInfo->dirID;)
	Macixposh(lpspathUdr->lpszFilePath = lpSzPathUdr;)
	NotMacixposh(lpspathUdr = lpSzPathUdr;)
		...
		short FAR *lpfReadonly)
{
	...
	*lpfReadonly = fFalse;
	...
	NotMacixposh(lpSzPath = (CHAR FAR *)lpspathUdr);
	Macixposh(lpSzPath = lpspathUdr->lpszFilePath);
	...
	*/
	...
	if (cbFile)
		...
		if (cbReading > 0)
			...
			for (i = 0; i < cbMac; i++)
				...
				else if ((ch == CR) && !fCr NotMacixposh(&& (*pch == LF)))
					{
					fCr = fTrue;
					}
				}
	...
	Macixposh(lpUdrInfo->volRefNum = lpspathUdr->volRefNum;)
	Macixposh(lpUdrInfo->dirID = lpspathUdr->dirID;)
