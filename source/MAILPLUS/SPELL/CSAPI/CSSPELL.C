	...
	*(lpsz + lpWordInfo->cchWordStripped) = 0;
	...
	if (!iStatus)
		...
		while (--iStatus >= 0)
			...
			NotMacixposh(rgbScrBuf[cbScr++] = LF);
		...
		while (lpb < lpbLim)
			...
			if (fCheckUdr =
				...
				BltBO((CHAR FAR *)WordInfo.rgb, (CHAR FAR *)rgbScrBuf, 
					cbWord);
				rgbScrBuf[cbWord] = CR;
				NotMacixposh(rgbScrBuf[cbWord + 1] = LF);
				}
				...
				if (lpUdrInfoExc && fCheckUdr)
					...
					if (
						...
						if (i > 0 || j < cbWord)
							...
							NotMacixposh(rgbScrBufT[cbWordT + 1] = LF);
			...
			if (fTrailingDot && !fWordConsideredAbbr)
				...
				NotMacixposh(rgbScrAbbr[cbWord] = LF);
				}
						...
						LPMDRS	lpMdrs)
{
	...
	*/
	NotMacixposh(lpSzPathMdr = (CHAR FAR *)lpspathMain);
	Macixposh(lpSzPathMdr = lpspathMain->lpszFilePath);
...
L_cleanupMdrInfo:
	...
	NotMacixposh(lpSzPathUdr = (CHAR FAR *)lpspathExc);
	Macixposh(if (lpspathExc) lpSzPathUdr = lpspathExc->lpszFilePath);

	if (Macixposh(lpspathExc &&) lpSzPathUdr && *lpSzPathUdr)
		{
		short fReadonly;
	...
	lpMdrInfo->volRefNum = Macixposh(lpspathMain->volRefNum) NotMacixposh(0);
	lpMdrInfo->dirID = Macixposh(lpspathMain->dirID) NotMacixposh(0L);
...
GLOBALSEC SpellVerifyMdr(	LPSPATH	lpspathMdr,
							...
							LID	FAR *lpLid)
{
	Macixposh(#pragma unused(lidExpected))
	...
	if (!(hFile = HFileWizFileOpen(NotMacixposh((CHAR FAR *))lpspathMdr,
