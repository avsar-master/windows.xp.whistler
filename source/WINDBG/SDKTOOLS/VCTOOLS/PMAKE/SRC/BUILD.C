...
*  19-Dec-2000 SB nmake /z requests
*  13-Dec-2000 SB nmk /n fix; refixed
*  08-Dec-2000 SB Fixed NMAKE111 bug #65 (extra rule when ::) and #66 (-error
*		  level fix for NMK)
*  06-Dec-2000 SB removed 'register' for -Oe; removed C4201 (for -Oe)
*  04-Dec-2000 SB Avoid c6 warning in redirect() by using ?: condition
*  01-Dec-2000 SB Plugged leak in starList and questionList; processTree()
*  22-Nov-2000 SB Changed free() to FREE(); findRule handles quoted targets
*  17-Nov-2000 SB changes to generated Labels for -z (capitalized)
*  15-Nov-2000 SB canonCmdLine() canonacallizes for -z option in doCommands()
*  13-Nov-2000 SB removed unreferenced variable in freeRules(); restoreEnv()
*		  declaration removed (unused)
*  07-Nov-2000 SB $(MAKE) cmd line to be echoed if -n not specified
*  19-Oct-2000 SB Fix timestamp for DoubleColon case
*  19-Oct-2000 SB Fixed *s = '"' in nextToken to '==';
*		      incorrect handles being freed and so searchHandle param
*  08-Oct-2000 SB '::' case handled; handle quoted filenames; modify nextToken
*  02-Oct-2000 SB add dynamic inline file handling support
*  14-Sep-2000 SB checkCommandFile() restored; add flag in execLine() for -z
*  13-Sep-2000 SB Add status check of emulation, error returned by cd avoided
*  07-Sep-2000 SB checkCommandLine() was replacing redirection by ' ' for -z
*		      in DOS
*  04-Sep-2000 SB Fix -z echoing and redirection problems using =d, =n & =c
*  24-Aug-2000 SB Check for cyclic dependency added.
*  16-Aug-2000 SB added lots of #ifdef DEBUG's; Added out of handles warning
*  03-Aug-2000 SB fix findRule() to get correct value for dir\foo.obj
*		      (.c.obj rule)
*  25-Jul-2000 SB With -n and -z execLine() attempts to emulate cd/chdir/set
*  20-Jul-2000 SB buildArgumentVector() was not handling "cmd     " correctly
*  14-Jul-2000 SB Commented out some code active for debug version.
*  30-Jun-2000 SB Call curTime() instead of time(), because C Runtime time
*		  format is different from DOS time format (offset from Jan 1,
*		  1970 instead of Jan 1, 1980 as in DOS)
*  29-Jun-2000 SB NMAKE -z now puts in echo commands to echo commands when the
*		  the shell executes.
*  05-Jun-2000 SB Free search Handles by call to findNext()
*  21-May-2000 SB Allow .c{dir}.obj rules now, starting {} is optional
*  18-May-2000 SB Added parameter to freeRules(); postpone target timestamp
*  15-May-2000 SB pFirstDep assigned at correct time in build()
*  13-May-2000 SB Added removeQuotes() function before calling $(MAKE)
*  03-May-2000 SB Suffixes compared ignoring case now in sortRules()
*  01-May-2000 SB changed OS/2 version 1.2 support, FILEINFO becomes void *
*  14-Apr-2000 SB added 'errorlevel' stuff for -z, corrected -n behaviour
*		  calls restoreEnv() to have macro back inheritance
*  04-Apr-2000 SB made functions NEAR to put all NMAKE functions into 1 module
*  09-Mar-2000 SB changed params for findRule() calls to FILEINFO **. findRule
*		      now calls QueryFileInfo instead of DosFindFirst in order
*		      to handle non-FAPI nature
*  01-Mar-2000 SB ren TheFirstDependent() to getFirstDep()
*  28-Feb-2000 SB time stamp is maxDepTime only for pseudoTarget with no Cmds
*  27-Feb-2000 SB doCommands() changed to handle macro expansions correctly
*		      for iterate commands
*  23-Feb-2000 SB modified buildArgumentVector() to allow arguments with
*		  embedded quotes.
*  21-Feb-2000 SB pFirstDep is either the file from inference rules or the
*		      first dependent or the first dependent specified
*  14-Feb-2000 SB Removed unrequired #ifdef BOUND for <os2.h>
*  03-Feb-2000 SB Changed functionality so that NMAKE does not have a escape
*		      character for command Lines, in execLine()
*  02-Feb-2000 SB Renamed findExt() to more meaningful skipPathList() & added
...
*  31-Jan-2000 SB Renamed variables and added function headers to findRule()
*  29-Jan-2000 SB Removed clobbering of MAKEFLAGS to allow their inheritance
*  25-Jan-2000 SB removed pCmdLineCopy and changes to buildArgumentVector as
*		      it caused "don't know how to make ''" errors
*  23-Jan-2000 SB changed target time to maximum of dependent times
