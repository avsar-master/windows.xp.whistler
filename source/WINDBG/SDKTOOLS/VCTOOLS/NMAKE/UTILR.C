...
*  04-Dec-2000 SB removed unreferenced local variables in findFirst()
*  01-Dec-2000 SB changed a remaining free() to FREE(); now FREE()'ing all
*		  allocated stuff from findFirst() on exit
*  22-Nov-2000 SB Add #ifdef DEBUG_FIND to debug FIND_FIRST, etc.
*  13-Nov-2000 SB Define INCL_NOPM to exclude <pm.h>
*  19-Oct-2000 SB findFirst() and findNext() get extra parameter
*  08-Oct-2000 SB remove quotes around a name before making system call
*  02-Oct-2000 SB setdrive() proto change
*  04-Sep-2000 SB Add DOSFINDCLOSE calls is findFirst and QueryFileInfo
*  05-Jul-2000 SB add curTime() to get current time. (C Runtime function
*		  differs from DOS time and so time() is no good
*  05-Jun-2000 SB call DosFindClose if DosFindNext returns an error
*  28-May-2000 SB Add getCurDir() to initialize MAKEDIR macro
*  24-Apr-2000 SB made FILEINFO a thing of the past. Replace by void *
*		  added OS/2 ver 1.2 support
*  05-Apr-2000 SB made all funcs NEAR; Reqd to make all function calls NEAR
*  09-Mar-2000 SB Added function QueryFileInfo() because DosFindFirst has FAPI
