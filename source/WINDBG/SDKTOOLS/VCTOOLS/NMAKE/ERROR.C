...
*  07-Dec-2000 SB Changed CopyRight Years to 1988-90; Add #ifdef DEBUG_HEAP
*  06-Nov-2000 SB Error messages now show NMAKE and not argv0
*  04-Sep-2000 SB heapdump() has two extra parameters
*  18-Aug-2000 SB For -z nothing done by makeMessage; Fix later
*  05-Jul-2000 SB localize rest of the messages in makeError()
*  14-Jun-2000 SB modified to localize all messages and auto update version no
*  05-Jun-2000 SB modified heapdump(), has a previous member in the list too
*  14-Apr-2000 SB modified heapdump() for better error messages when DEBUG
*  05-Apr-2000 SB made functions NEAR; all funcs to one code segment
*		  modified heapdump() to give better heap violations
*  22-Mar-2000 SB del call to unlinkTmpFiles() ;add call to delScriptFiles().
*  17-Mar-2000 SB heapdump() has an additional check built-in
*  10-Mar-2000 SB Changed makeMessage() for -z to get echo CMD's into PWB.SHL
*  16-Feb-2000 SB changed makeError() and makeMessage() to handle -help
*  09-Jan-2000 SB changes in makeError() to handle -help correctly
