...
//
//CG: special case behavior for InPlace UI
//
extern BOOL gfInPlace;

void
CalcWnds(int disptools, int displine, int dispcolor, int disppaint)
{
   int i;
   ...
   if(gfInPlace)
   {
       clientwid = pbrushRct[PAINTid].right - pbrushRct[PAINTid].left;
       clienthgt = pbrushRct[PAINTid].bottom - pbrushRct[PAINTid].top;
       gapwid = gaphgt = 0;
   }
   else
   {
   ...
   if(!gfInPlace && ((disptools == SHOWWINDOW) ||
   ...
   if(!gfInPlace && ((dispcolor == SHOWWINDOW) ||
