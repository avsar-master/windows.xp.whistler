...
#ifdef JAPAN                // added by Hiraisi  19 May. 2000
        iDigits = 2;
#else
...
LONG
NumToPels(LONG lNum, BOOL bHoriz, BOOL bInches)
{
   int nLogPixels;
   HDC   hDC;
...
LONG
PelsToNum(LONG lNum, BOOL bHoriz, BOOL bInches)
{
   int nLogPixels;
   HDC   hDC;
   LONG  lResult;
...
BOOL
StrToNum(LPTSTR num, LONG FAR *lpNum)
{
   LPTSTR s;
   LONG  lNum = 0;
   BOOL  fSign;
...
BOOL
GetDlgItemNum(HWND hDlg, int nItemID, LONG FAR * lpNum)
{
   TCHAR num[20];
...
BOOL
NumToStr(LPTSTR num, LONG lNum, BOOL bDecimal)
{
   TCHAR szFormat[10];
...
BOOL
SetDlgItemNum(HWND hDlg, int nItemID, LONG lNum, BOOL bDecimal)
{
   TCHAR  num[20];
