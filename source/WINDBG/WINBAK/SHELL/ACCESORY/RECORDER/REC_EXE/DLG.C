...
HISTORY
    ...
    sds		25 Mar 2000	MsgString now NEVER reads beyond segment end
				better oversize dlgCmds handling
    sds		29 Mar 2000	Now selecting first event in dlgCmds
				pref/autoload now saves full file path
    sds		30 Mar 2000	Now disallows ctrl/alt hot keys until
				they actually work in Windows
				LdStr with hWnd arg
    sds		31 Mar 2000	bug fix in dlgPref
    sds		6 Apr 2000	now clearing state buffer for ToAscii
    sds		26 Apr 2000	deals with B_FORCEACTIVE bit
    sds		27 Apr 2000	different break types for int dlg box
				new desktop naming for event display
    sds		28 Apr 2000	goes to hourglass cursor faster on editmacro
    sds		5 May 2000	now using DESKTOPMODULE magic
    sds		9 May 2000	now using window to xlate virtkey to ANSI
    sds		11 May 2000	WIN2 fix
    sds		13 May 2000	now minimizes on playback from dlgbox
    sds		16 May 2000	fix for discarding macro code
		18 May 2000	massive user intfc changes
    sds		22 May 2000	fixed hotkey display in PBError (win3)
    sds		23 May 2000	fixed bug in not loading macro code
    sds		30 May 2000	system menu support
    sds		31 May 2000	CLOSE record suspension now resumes
    sds		7 Jun 2000	now cleaning out system menu, no
				SC_CLOSE needed
    sds		9 Jun 2000	name change to recorder
    sds		12 Jun 2000	WIN2 changes
				now deleting sysmenu separators
    sds		3 Jul 2000	FixSysMenu no longer needed for Win30
			    	now using wsprintf
    sds		27 Jul 2000	WriteProfile to szRecorder, not szCaption
    sds		8 Aug 2000	MsgString now clears SHIFT for TranslateMsg
    sds		17 Aug 2000	error dlgbox now supports save on TOOLONG
    sds		17 Aug 2000	AddHotKey now takes id
    sds		28 Aug 2000	fixed bug displaying wrong instruction number
				for Desktop Msg
    sds		20 Sep 2000	ABOUT now comes down on CANCEL
				removed bogus GetKeyNametext call
    sds		21 Sep 2000	better handling of pb error during recording
    sds		22 Sep 2000	MsgString now clears ALT and CTRL before
				  TranslateMsg - cleaner xlate
    sds		30 Oct 2000	fixed bug losing hotkey
    sds		31 Oct 2000	fixed bug with B_NESTS flag
    sds		6 Dec 2000	altered wait cursor handling in dlgEdit
    sds		19 Dec 2000	now deletes CLOSE option on dlgInt sysmenu
    sds		26 Dec 2000	now checks for duplicate comments
    ...
    sds 	8 Jan 2000	fixed bug in MsgString
