...
HISTORY
    ...
    sds		25 Mar 2000	now loading ERRRESOURCE
    sds		28 Mar 2000	szTempDir
    sds		29 Mar 2000	cmd line improvements
    sds		30 Mar 2000	now loads E_NOMEM string
    sds		6 Apr 2000	TDYN version now distinct from file
    sds		27 Apr 2000	loads M_DESKINT
    sds		3 May 2000	(except if WIN2)
    sds		5 May 2000	now minimizes if have cmd line hot key
    sds		5 May 2000	removed szDeskInt load
    sds		9 May 2000	now creates Keybd xlate window
    sds		16 May 2000	setting bProtMode
    sds		18 May 2000	WIN2 change, REMOTECTL
    sds		22 May 2000	now passing cmd line to previous Tracer
    sds		24 May 2000	moved remote ctl code to REMOTE.C
    sds		25 May 2000	now passes cwd on cmd line to other Tracer
    sds		26 May 2000	fixed no-caption bug, WIN2 upgrade
    sds		9 Jun 2000	name change to Recorder, LB initially disabled
    sds		12 Jun 2000	bProtMode now bNotBanked, WIN2 changes
    sds		16 Jun 2000	fixed display problem loading listbox
				  at init time
    sds		27 Jul 2000	now using AnsiUpper, not toupper
				loading .hlp
				WIN.INI strings now hard coded
    sds		4 Aug 2000	now restores prev instance
    sds		25 Aug 2000	removed "include winexp.h", in windows.h now
    sds		18 Sep 2000	now using WinFlags to set bNotBanked
    sds		19 Sep 2000	removed unused error msg reference
    sds		20 Sep 2000	now using COLOR_WINDOW for bckgnd brush
    sds		20 Sep 2000	winexp.h restored for win2
    sds		28 Sep 2000	background brush now NULL
    sds		5 Oct 2000	listbox now NOINTEGRALHEIGHT
    sds		10 Oct 2000	cmdline from new instance now better about
    ...
    sds 	8 Jan 2000	fixed bug with "-h {hot key}" parsing
				MSC 6 fixups
