/****************************Module*Header******************************\
* Copyright (c) 1987 - 2000  Microsoft Corporation                      *
*                                                                       *
...
#ifdef JAPAN            //  added 06 Apr. 2000  by Hiraisi
#undef NOSYSMETRICS
#endif
...
BOOL PRIVATE Text2DPInit(HWND hWnd)
{
   HFONT hOldFont;

#ifdef JAPAN              //  added 01 Sep. 2000  by Hiraisi
...
#ifdef JAPAN      // added 14 Apr. 2000 by Hiraisi
    ...
    int Len)
{
    ABC abc;
    SIZE sz;
#ifdef JAPAN     //  added 16 Apr. 2000  by Hiraisi
    int width;
#endif
...
int PRIVATE MakeText(HDC paintDC, int x, int y,
        ...
        BOOL bShadow, BOOL bOutline, int Action)
{
    ...
    xx = x;
    yy = y;
    if (bShadow) {xx -= 4; width += 5;yy += 4; height += 5;}
    else if (bOutline) { xx -= 1; width += 2;
                         yy -= 1; height += 2;}
    ...
    if (rect1.left > xx) rect1.left = xx;
    if (rect1.right < xx + width) rect1.right = xx + width;
    if (rect1.top > yy) rect1.top = yy;
    if (rect1.bottom < yy + height) rect1.bottom = yy + height;
...
#ifdef PENWIN // NOTE: there is a matching ifdef for the closing brace
...
#if defined(JAPAN) || defined(KOREA) //  added 10 Apr. 2000  by Hiraisi
...
#if defined(JAPAN) || defined(KOREA)     //  added 15 Apr. 2000  by Hiraisi
        if( flagVert )
...
#ifdef JAPAN //KKBUGFIX       // added by Hiraisi  04 Sep. 2000 (in Japan)
    case WM_RESETCARET:
        CreateCaret(hWnd, (HBITMAP)NULL, xCaret, yCaret);
        SetCaretPos(endPt.x, endPt.y);
        ShowCaret(hWnd);
