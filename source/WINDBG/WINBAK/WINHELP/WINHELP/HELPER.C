...
* 02-Jan-2000 LeoN    Move Title Setting call to FFocusSzHde
* 04-Jan-2000 LeoN    Re-enable back button with secondary windows present by
*                     moving the zero-out of the button hwnds to
*                     CreateCoreButtons
* 04-Jan-2000 LeoN    Remove force param from EnableDisable
* 15-Jan-2000 LeoN    Handle file not fount in key searches in ExecAPI
* 19-Jan-2000 LeoN    Clean up some confusion stuff relating to repainting
*                     the button window.
* 21-Jan-2000 RobertBu  Turn logo functionality on during file change in case
*                       no file is loaded and the load fails.
* 21-Jan-2000 LeoN      FReplaceHde requires a DE or an FM. Ensure that is
*                       handled correctly in FRepalceCloneHde. Add GI_CURFM
* 22-Jan-2000 LeoN      Invalidate current window member name if changing
*                       files.
* 30-Jan-2000 LeoN      Always destroy dialogs in ExecApi (avoids secondary
*                       window issues re removing the window underneath.)
* 31-Jan-2000 LeoN      Add fForce flag back to EnableDisable
* 01-Feb-2000 LeoN      Member names are now passed as near strings to avoid
*                       DS movement problems.
* 02-Feb-2000 RobertBu  Added code to handle failure of GetDC()
* 05-Feb-2000 LeoN      Added code to disable main help windows around
*                       dialog calls.
* 07-Feb-2000 LeoN      Added GI_FFATAL
* 08-Feb-2000 LeoN      Change enable scheme to only change the non-current
*                       window. Current window is handled by dialog, and
*                       doesn't get focus back if we do it ourselves.
* 08-Feb-2000 LeoN      Remove LHAlloc Use
* 08-Feb-2000 RobertBu  Fixed problem where MouseInTopicHde() was not getting
*                       a valid DC.
* 2000/02/11  kevynct   Added call to turn off hotspots if we jump.
* 2000/02/15  kevynct   H3.5 917: a WORD was being assigned an INT value in
*                       SizeWindows();
* 22-Feb-2000 LeoN      Move actual QuitHelp() call made in response to an
*                       API message into ExecAPI, and have Dispatch proc
*                       allow cmdQuit to get posted as a WM_EXECAPI msg.
* 25-Feb-2000 LeoN      Bugs 949 and 951. Changes to the way that
*                       FReplaceCloneHde handles same-fm scenario.
* 08-Mar-2000 LeoN      HELP31 #979: Don't run config macros when cloning.
* 15-Mar-2000 LeoN      MouseInFrame => MouseInTopicHde. Remove a few
*                       now pointless asserts.
* 02-Apr-2000 LeoN      HELP31 #1012: Don't bring up iconized help when
*                       closing
* 05-Apr-2000 LeoN      Don't EnableDisable when just resizing (in Goto).
...
*                       elsewhere.
* 09-Apr-2000 LeoN      Remove a couple of W4 warnings
* 15-Apr-2000 LeoN      Remove a couple of new W4 warnings
* 02-Apr-2000 RobertBu  Removed CBT support
* 04-Apr-2000 RobertBu  Added code to handle partial key searches, window
*                       positioning through the API (#1009, #1037).
* 22-Apr-2000 LeoN      HELP31 #1050: Don't set the focus to an invisible
*                       winhelp
* 01-May-2000 LeoN      Remove cmdTerminate
* 01-May-2000 LeoN      HELP31 #1081: Add fNoHide.
* 15-May-2000 LeoN      HELP31 #1078: Clean up the way we destroy dialogs
* 16-May-2000 LeoN      HELP31 #1063: DisposeFm in FReplaceCLoneHde
* 27-May-2000 LeoN      HELP31 #1136: Pass correct fm to szpartsfm, in case
*                       older one had been discarded.
* 29-May-2000 LeoN      Cleanup in prep for code review.
* 30-May-2000 LeoN      HELP31 #1034: pass additional param on DW_CHGFILE
* 05-Jun-2000 LeoN      HELP31 #1165: ensure that we get an hde before
*                       requesting a palette from it.
* 03-Jul-2000 LeoN      HELP31 #1176: if a null filename pointer is passed in
*                       an API call, ensure that the offset field is null.
* 05-Jul-2000 LeoN      HELP31 #1188: Assume MAIN window member unless
*                       otherwise spec'd
* 05-Jul-2000 LeoN      HELP31 #1201: Pay attention to FDestroyDialogsHwnd
*                       return value in DispatchProc.
* 25-Jul-2000 Dann      Call ResultsButtonsStart after layout but before drawing
*                       so we can tell what state of Next/Prev buttons should be
* 25-Jul-2000 LeoN      HELP31 #1233: look for "default" keyword only for
*                       multi-key search.
* 27-Aug-2000 LeoN      HELP31 #1078: Clear handle to search set after
*                       freeing, so as not to try and free it again.
* 27-Aug-2000 LeoN      HELP31 #1245: rctHelp applies ONLY to the main window
* 08-Sep-2000 RussPJ    3.5 #231 & 338 - Restoring to maximized state.
* 24-Sep-2000 JahyenC   3.5 #5 - Replaced direct SetHds calls in Goto() with
*                       GetAndSetHDS()/RelHDS() pair.
* 17-Sep-2000 JahyenC   3.5 #16 - re: replacement of copyright string static
*                       storage with dynamic.  Copyright string is copied out
*                       a bit differently in GetCopyright
* 22-Oct-2000 RussPJ    3.5 #628 - preventing multiple simultaneous dialogs.
* 28-Oct-2000 RussPJ    3.5 #146 - Fixed logic to trigger ForceFile API.
* 06-Nov-2000 RussPJ    3.5 #373 - Using NULL instead of hidden window for
*                                  dialog parent.
* 06-Nov-2000 BethF     3.5 #589 - Added UpdateWinIniValues() function.
* 12-Nov-2000 RussPJ    3.5 #644 - Getting file from executable directory.
* 08-Jan-2000 LeoN    HELP31 #1226: correct DS releasing order to avoid RIP
* 13-Jan-2000 LeoN    HELP31 #1386: ensure that config macros not run
*                     repeatedly
* 22-Jan-2000 LeoN    HELP31 1357: Ensure that button and menu bindngs are
*                     released only at the correct times.
* 22-Jan-2000 LeoN    HELP31 #1405: Check the timestamp of the help file on
*                     API messages to make sure it hasn't changed.
* 23-Jan-2000 LeoN    HELP31 #1406: Ensure config macros run at the right
*                     time.
* 24-Jan-2000 LeoN    HELP31 #1405: Use wERRA_LINGER for changed file test
* 02-Mar-2000 RussPJ  3.5 #569 - Leaving scroll bars alone on partial
*                     keyword API.
* 03-Apr-2000 RussPJ  3.5 #711 - showing window on cmdFocus API.
* 03-Apr-2000 RussPJ  3.5 #712 - killing us on failed partial key.
* 03-Apr-2000 RussPJ  3.5 #713 - setidx when hidden shows wash.
* 03-Apr-2000 RussPJ  3.5 #742 - Only creating core buttons when changing files.
* 04-Apr-2000 LeoN    HELP31 #1308: Add GI_MEMBER
* 06-Apr-2000 RussPJ  3.5 #741 - Serializing APIs to detect partial key re-enter
*
*****************************************************************************/
...
QV      qv
   ...
   * REVIEW: Which ones, when and why? 27-May-2000 LeoN
   */
  switch(fWhich)
    {
  case fGOTO_CTX:
...
BOOL    fReplace
   ...
   * past this point. 27-May-2000 LeoN
   */
