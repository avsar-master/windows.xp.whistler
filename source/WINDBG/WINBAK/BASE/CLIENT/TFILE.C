    ...
    )
{
    ...
    *buff = '\0';
    ...
    CloseHandle(iFile);
    xassert(CopyFile("e:\\xp\\dll\\usersrv.dll",buff,FALSE));
    DeleteFile("xyzzy");
    ...
    *buff = '\0';
    ...
    attr = GetFileAttributes("xpos3.cfg");
    xassert(attr == (FILE_ATTRIBUTE_HIDDEN | FILE_ATTRIBUTE_ARCHIVE));

    xassert(SetFileAttributes("xpos3.cfg", FILE_ATTRIBUTE_NORMAL));
    ...
    xassert(!stricmp("e:\\xp\\bin\\ls.exe",&reopen1.szPathName[0]));
    CloseHandle(iFile);
    reopen2 = reopen1;
    ...
    xassert(iFile && iFile != INVALID_HANDLE_VALUE);
    xassert(!stricmp("e:\\xp\\bin\\ls.exe",&reopen2.szPathName[0]));
    CloseHandle(iFile);
    iFile = (HANDLE)OpenFile("ls.exe",&reopen2, OF_EXIST );
    xassert(!CloseHandle(iFile));
    xassert(CopyFile("e:\\xp\\bin\\ls.exe","xxx.xxx",FALSE));
