 ...
 *
 *  History:
 *      29-Oct-2000 bens    Initial version
 *      16-Jan-2000 bens    Fixed EOF bug; do not report EOF from TFEof until
        ...
        while (ptf->pchRead <= ptf->pchLast) {  // Copy chars from buffer
            ...
            default:
                if (cbBuffer <= 1) {    // No room for NULL terminator
                    *pch = '\0';        // Terminate buffer, to be nice
// BUGBUG 29-Oct-2000 bens Should we read to end-of-line?
