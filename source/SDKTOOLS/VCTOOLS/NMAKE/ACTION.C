...
*  08-Dec-2000 SB removed local used without initialization warnings for -Oes
*  07-Dec-2000 SB removed register to compile using C6 -Oes (warning gone)
*  06-Dec-2000 SB changed expandFileNames() type to void instead of void *
*  22-Nov-2000 SB Changed free() to FREE()
*  13-Nov-2000 SB Removed an unreferenced local in endNameList()
*  02-Oct-2000 SB add dynamic inline file handling support
*  04-Sep-2000 SB Add A_DEPENDENT and fix macro inheritance
*  24-Aug-2000 SB Allow $* on dependency lines
*  14-Jul-2000 SB Environment macro was getting updated even when CMDLINE
*		  macro was present
*  29-Jun-2000 SB addItemToList() now maintains Global inline file List
*  26-Jun-2000 SB Fixed -e for recursive NMAKE
*  22-May-2000 SB NZ options work independently. -NZ does both stuff now
*  13-May-2000 SB Changed delList to contain just names of files and no "del "
*  14-Apr-2000 SB made targetList NEAR, no 'del inlinefile' cmd for -n now
*  05-Apr-2000 SB Added parameters to makeRule() & makeTarget(); this get rid
*		  of globals
*  03-Apr-2000 SB changed all functions to NEAR to get them into one module
*  21-Mar-2000 SB changed assignDependents() and assignCommands() to handle
*		  multiple target case correctly.
*  20-Mar-2000 SB startNamelist() doesn't flag error if target macro is null
*		  and >1 target given; commented startNameList()
*  16-Feb-2000 SB addItemToList() now appends to delList instead of List so
*		  that all 'del scriptFile' cmds can be at the end of the make
*  29-Jan-2000 SB added targList but not used as yet
*  19-Jan-2000 SB changed startNameList() to avoid GP fault, bug# 162
*  18-Jan-2000 SB modified endNameList(), added makeList() and makeBuildList()
