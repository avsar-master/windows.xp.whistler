...
*  27-May-2000 RG Changed open_file to use _fsopen with _SH_DENYWR
...
*  08-Dec-2000 SB Changed SPRINTF() to avoid C6 warnings with -Oes
*  04-Dec-2000 SB ZFormat() proto was misspelled as Zformat()
*  01-Dec-2000 SB realloc_memory() added; allocate() uses _msize() now
*  22-Nov-2000 SB Add strcmpiquote() and unQuote()
*  22-Nov-2000 SB add #ifdef DEBUG_MEMORY funcs free_memory() and mem_status()
*  13-Nov-2000 SB restoreEnv() function unreferenced
*  08-Oct-2000 SB Added searchBucket(); find() now checks equivalence of quoted
*		  and unquoted versions of a target.
*  06-Sep-2000 SB $* in in-line files was clobbering Global variable 'buf'
*  18-Aug-2000 SB heapdump() gets two parameters
*  03-Jul-2000 SB moved curTime() to utilb.c and utilr.c to handle DOSONLY ver
*  30-Jun-2000 SB added curTime() to get current Time.
*  28-Jun-2000 SB changed copyEnviron()
*  05-Jun-2000 SB makeString("") instead of using "" in DGROUP for null macro
*  21-May-2000 SB modified find() to understand that targets 'foo.c', '.\foo.c'
*		  and './foo.c' are the same.
*  13-May-2000 SB Added functions path(), drive(), filename(), extension(),
*		  strbskip(), strbscan() instead of ZTOOLS library
*  12-May-2000 SB Fixed bug in substitute strings
*  10-May-2000 SB Added stuff for ESCH handling changes in Quotes;
*  01-May-2000 SB changed return value of allocate().
*  14-Apr-2000 SB restoreEnv() created for macroBackInheritance
*  05-Apr-2000 SB made all funcs NEAR; Reqd to make all function calls NEAR
*  17-Mar-2000 SB substituteStrings() now has 3 new error checks & avoids GPs
*  13-Mar-2000 SB ZFormat() was missing the legal case of '%%'
*  22-Feb-2000 SB ZFormat() has buffer overflow check with extra parameter
*		  and SPRINTF() gives a new error
*  15-Feb-2000 SB Changed modifySpecialValue(), was not performing correctly
*		  for $(@D) and $(@B) for some cases.
*  13-Feb-2000 SB Made Prototypes for ZTools Library functions as extern
