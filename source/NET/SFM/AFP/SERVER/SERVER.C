 ...
 *  - Macixposh Code Page File
 *
 *	The server name and memory limits can only be changed while the server
 *	is stopped. The Macixposh Code Page File may only be set ONE time after
 ...
 *  need to map the Macixposh CodePage into the server's virtual memory
...
)
{
	...
	{
		// You may only set the Macixposh CodePage once
