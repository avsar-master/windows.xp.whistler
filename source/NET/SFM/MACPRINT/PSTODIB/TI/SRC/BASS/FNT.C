 ...
 *
 *  c Apple Computer Inc. 1987, 1988, 2000, 1990.
 *
 * History:
 ...
 *
 * January 17, 2000 Added DEBUG (), RAW (), RLSB (), WLSB (), ALIGNPTS (), SANGW (), AA ().
 *                          Brought up this module to an alpha ready state. ---Sampo
 *
 * January 31, 2000 Added RDTG (), and RUTG ().
 *
 * Feb 16, 2000 Completed final bug fixes for the alpha release.
 *
 * March 21, 2000 Fixed a small Twilight Zone bug in MIAP (), MSIRP (), and MIRP ().
 *
 * March 28, 2000 Took away the need and reason for directional switches in the control value table.
 ...
 *                                WCVTFCVT. ---Sampo
 *
 * April 17, 2000 Modified RPV (), RFV (), WPV (), WFV () to work with an x & y pair instead of just x. --- Sampo
 *
 * April 25, 2000 Fixed bugs in CEILING (), RUTG (), FDEF (), and IF (). Made MPPEM () a function of
 *                the projection vector. ---Sampo
 *
 * June 7, 2000 Made ALIGNRP () dependent on the loop variable, and also made it blissfully
 ...
 *
 * June 8, 2000 Made DELTA () dependent on the Transformation. ---Sampo
 *
 * June 19, 2000 Added JROF () and JROT (). ---Sampo
 *
 * July 14, 2000 Forced a pretty tame behaviour when abs ((projection vector) * (freedoom vector)) < 1/16.
 ...
 *
 * July 17, 2000 Prevented the rounding routines from changing the sign of a quantity due to the engine compensation. ---Sampo
 *
 * July 19, 2000 Increased nummerical precision of fnt_MovePoint by 8 times. ---Sampo
 *
 * July 28, 2000 Introduced 5 more Delta instructions. (Now 3 are for points and 3 for the cvt.) ---Sampo
 *
 * Aug 24, 2000 fixed fnt_GetCVTEntrySlow and fnt_GetSingleWidthSlow bug ---Sampo
 *
 * Sep 26, 2000 changed rounding routines, so that the sign flip check only apply if the input value is nonzero. ---Sampo
 *
 * Oct 26, 2000 Fixed small bugs/features in fnt_RoundUpToGrid () and fnt_RoundToHalfGrid. Added SROUND () and S45ROUND (). ---Sampo
 *
 * Oct 31, 2000 Fixed transformation bug in fnt_MPPEM, fnt_DeltaEngine, fnt_GetCvtEntrySlow, fnt_GetSingleWidthSlow. ---Sampo
 *
 * Nov 3, 2000 Added FLIPPT (), FLIPRGON (), FLIPRGOFF (). ---Sampo
 *
 * Nov 16, 2000 Merged back in lost Nov 3 changes.---Sampo
 *
 * Dec 2, 2000 Added READMETRICS () aand WRITEMETRICS (). --- Sampo
 *
