...
/*  byte   huge *p,  huge *temptr,  huge *old_heap ; pj 4-18-2000 */
    byte   huge *p,  huge *old_heap ;
    ubyte  nobyte, packed_header ;
    fix16  l_size ;

    old_heap = vmheap ;
/*  cur_heap = first_heap = (struct heap_def huge *)get_heap() ; pj 4-30-2000 */
    ...
    */
    while (get_token(&ttemp, inp)) {
        ...
        if (TYPE(&ttemp) == EOFTYPE) { /* erik chen 5-1-2000 */
            ERROR(SYNTAXERROR) ;
            abort_flag = 1;
            return(FALSE);
        }
...
byte   FAR *string;
{
    ...
    byte        sign = 0, c;
    fix         i,j;
    byte        FAR *str;   /* erik chen 5-20-2000 */

        result_1 = 0;
        if ((c = *string) == '+')
            string++;
        else if (c == '-') {
            str = string;       /* erik chen 5-20-2000 */
            string++; sign++;
        }
...
/*      if(i > 9)
                return(get_real(token, string)); erik chen 5-20-2000 */
