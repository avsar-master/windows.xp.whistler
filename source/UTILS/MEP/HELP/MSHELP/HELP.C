...
*       08-Oct-2000 ln  Changes to improve the previous change to work (allow
...
*       19-May-2000 ln  Correct bug in decompressing, where we would not
*                       decompress if the tables didn;t exist.
*       12-Apr-2000 ln  Ensure that we handle Locks failing correctly. Also
*                       remove TossPortion usage. Unlock handles directly.
*       10-Mar-2000 ln  Change MapTopicToContext to look forward. Changed
*                       HelpNc to look begining at passed context string.
*       17-Jan-2000 ln  Correct creation of basename in HelpOpen to account
