...
/*
 * "@(#) NEC(MIPS) fillpath.c 1.1 94/06/07 12:02:14"
 *
 * Copyright (c) 1994 NEC Corporation.
 *
 * Modification history
 *
 * Create 1994.06.07    by T.Izumori
 *
 */
...
typedef VOID (*PFNFILL)(SURFOBJ *, ULONG, PRECTL, MIX, RBRUSH_COLOR, POINTL*, BRUSHOBJ*);
...
VOID vTrgBlt(SURFOBJ *, ULONG, PRECTL, MIX, RBRUSH_COLOR, POINTL *, BRUSHOBJ *);
VOID vColorPat(SURFOBJ *, ULONG, PRECTL, MIX, RBRUSH_COLOR, POINTL *, BRUSHOBJ *);
BOOL bIntersectRect(RECTL*, RECTL*, RECTL*);


/******************************Public*Routine******************************\
* BOOL DrvFillPath(pso, ppo, pco, pxo, pbo, pptlBrush, pla, mix)
*
* Fill the path.
\**************************************************************************/
        ...
        do {
            ...
            if (iLeftEdge < pCurrentEdge->X) {
                ...
                if (ulNumRects >= MAX_PATH_RECTS) {
                    ...
                    (*pfnFill)(pso, ulNumRects, prclRects, mix, rbc,
                               pptlBrush, pbo);
...
draw_remaining_rectangles:
    ...
    if (ulNumRects > 0) {
        (*pfnFill)(pso, ulNumRects, prclRects, mix, rbc, pptlBrush, pbo);
    }
...
PRECTL	wkRects;
	...
	while (ulNumRects > 0)
	{
            ...
            wkRects = &(prclRects[--ulNumRects]);

	    switch (mix &= 0xFF)
	    {
	    case R2_XORPEN:
		DrvpSolidColorFillXor(pso, wkRects, pbo);
		break;
	    case R2_COPYPEN:
	    case R2_WHITE:
	    case R2_BLACK:
		DrvpSolidColorFill(pso, wkRects, pbo);
		break;
...
PRECTL	wkRects;
	...
	while (ulNumRects > 0)
	{
	    ...
	    {
	    case R2_XORPEN:
		DrvpPatternFillXor(pso, wkRects, pbo);
		break;
	    case R2_COPYPEN:
	    case R2_WHITE:
	    case R2_BLACK:
		DrvpPatternFill(pso, wkRects, pbo);
		break;
