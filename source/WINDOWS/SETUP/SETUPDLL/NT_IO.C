    ...
    )
{
    ...
    if(XPDriveNameW.Buffer[(XPDriveNameW.Length/sizeof(WCHAR))-1] == (WCHAR)'\\')
    {
        XPDriveNameW.Buffer[(XPDriveNameW.Length/sizeof(WCHAR))-1] = 0;
        XPDriveNameW.Length -= sizeof(WCHAR);
    }

    memset(&oa, 0, sizeof(OBJECT_ATTRIBUTES));
    oa.Length = sizeof(OBJECT_ATTRIBUTES);
    oa.ObjectName = &XPDriveNameW;
    oa.Attributes = OBJ_CASE_INSENSITIVE;
    ...
    RtlFreeUnicodeString(&XPDriveNameW);
    return(NT_SUCCESS(status) ? Handle : NULL);
}
