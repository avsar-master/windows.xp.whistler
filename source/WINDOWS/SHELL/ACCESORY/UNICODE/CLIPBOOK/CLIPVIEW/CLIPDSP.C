...
/***************************************************************************/
/**                  Microsoft Windows                                    **/
/**            Copyright(c) Microsoft Corp., 2000,2000                    **/
/***************************************************************************/
// ts=4

// CLIPDSP.C - ClipBook viewer display routines adapted from 3.1 clipboard
// viewer.
...
void PASCAL ShowString( HWND, HDC, WORD);
...
BOOL  FAR  PASCAL  MyOpenClipboard(HWND hWnd)
{
  HDC   hDC;
  RECT  Rect;
...
void PASCAL SetCharDimensions(HWND hWnd, HFONT hFont)
{
  register HDC   hdc;
  TEXTMETRIC   tm;
  PMDIINFO   pMDI;
  ...
  pMDI->cxChar = tm.tmAveCharWidth;
  pMDI->cxMaxCharWidth = tm.tmMaxCharWidth;
  pMDI->cyLine = tm.tmHeight + tm.tmExternalLeading;
...
void PASCAL ClipbrdVScroll(HWND hwnd, WORD wParam, WORD wThumb)
{
  int       cyWindow;
  long      dyScroll;
  long      cyScrollT;
  ...
  switch (wParam)
    {
     ...
     cyScrollT = (LONG)(((LONG)wThumb * pMDI->cyScrollLast)
      / VPOSLAST);
     break;
  ...
  SetScrollPos(pMDI->hwndVscroll,
          SB_CTL,
          (pMDI->cyScrollLast <= 0) ? 0 : (int)((cyScrollT * (DWORD)VPOSLAST) / pMDI->cyScrollLast),
          TRUE);
}
...
void PASCAL ClipbrdHScroll(HWND hwnd, WORD wParam, WORD wThumb)

{
  ...
  SetScrollPos(pMDI->hwndHscroll,
          SB_CTL,
          (pMDI->cxScrollLast <= 0) ? 0 : (int)(((DWORD)cxScrollT * (DWORD)HPOSLAST) / (DWORD)(pMDI->cxScrollLast)),
          TRUE );
}
...
void PASCAL DibGetInfo(HANDLE hdib, LPBITMAP pbm)
{
  LPBITMAPINFOHEADER lpbi;
...
if (hdib)
   ...
   if (lpbi)
      ...
      fPalAvail = IsClipboardFormatAvailable(CF_PALETTE);
      ...
      SetDIBitsToDevice(hdc,
             ...
             fPalAvail ? DIB_RGB_COLORS : DIB_PAL_INDICES);
...
BOOL PASCAL FShowBitmap( HWND hwnd, HDC hdc,register PRECT prc,
                    HBITMAP hbm,int cxScroll,int cyScroll)
{
  ...
  GetObject((HBITMAP)hbm, sizeof(BITMAP), (LPVOID)&bitmap);
  ...
  if (pMDI->cyScrollLast == -1)
    {
      /* Compute last scroll offset into bitmap */
     pMDI->cyScrollLast =
      bitmap.bmHeight - (pMDI->rcWindow.bottom - pMDI->rcWindow.top);
      if (pMDI->cyScrollLast < 0)
     pMDI->cyScrollLast = 0;
    }
  ...
  if ( pMDI->cxScrollLast == -1)
    {
      /* Compute last scroll offset into bitmap */
     pMDI->cxScrollLast =
      bitmap.bmWidth - (pMDI->rcWindow.right - pMDI->rcWindow.left);
      if ( pMDI->cxScrollLast < 0)
     pMDI->cxScrollLast = 0;
    }
...
DrawText(hdc, szBuffer, -1, &(GETMDIINFO(hwnd)->rcWindow),
      DT_CENTER | DT_WORDBREAK | DT_NOJWORDBRK | DT_TOP);
#else
DrawText(hdc, szBuffer, -1, &(GETMDIINFO(hwnd)->rcWindow),
...
BOOL         fUnicode)
{
  ...
  if (!fUnicode)
      ...
      {
         lpch += LOWORD(CchLineW(pMDI, hdc, (WCHAR *)rgch, (WCHAR *)lpch,
  ...
  while (cLine--)
     ...
     if (!fUnicode)
        {
        TextOut(hdc, rc.left, yLine, (WCHAR *)rgch, HIWORD(lT));
        lpch += wLen;
        }
     else
        {
        if (!TextOutW(hdc, rc.left, yLine, (WCHAR *)rgch, HIWORD(lT)))
           GetLastError();
        lpch += wLen * sizeof(WCHAR);
        }
     yLine += pMDI->cyLine;
     cLineAllText++;
     if ((!fUnicode && !*lpch) || (fUnicode && !*((WCHAR *)lpch)))
        break;
     }
  ...
  if (pMDI->cyScrollLast == -1)
     ...
     do {
        if (!fUnicode)
           ...
           lpch += LOWORD(CchLineW(pMDI, hdc, (WCHAR *)rgch, (WCHAR *)lpch, cchLineMax, wWidth)) * sizeof(WCHAR);
        cLineAllText++;
        }
        while ((!fUnicode && *lpch) || (fUnicode && *((WCHAR *)lpch)));
...
void PASCAL SendOwnerSizeMessage(HWND hwnd,int   left, int top,
...
register int iSize)
{
  ...
  *szName = TEXT('\0');
  ...
  if ((hrgch = GlobalAlloc(GMEM_MOVEABLE | GMEM_LOWER, ByteCountOf(iSize + 1))) == NULL) {
  ...
  case CF_OWNERDISPLAY:         /* Clipbrd owner app supplies name */
     *lprgch = TEXT('\0');
     SendOwnerMessage(WM_ASKCBFORMATNAME, (WPARAM)iSize, (LPARAM)lprgch);
  ...
  if ( pMDI->fDisplayFormatChanged )
     ...
     if (fOwnerDisplay)
        ...
        SendOwnerSizeMessage(hwnd,
            pMDI->rcWindow.left, pMDI->rcWindow.top,
            pMDI->rcWindow.right, pMDI->rcWindow.bottom);
        }
     else
        {
        /* Give the window a small margin, for looks */
        InflateRect(&(pMDI->rcWindow), -(int)(pMDI->cxMargin),
          -(int)(pMDI->cyMargin));
        }
...
void PASCAL SaveOwnerScrollInfo(register HWND   hwnd)

{
  GetScrollRange( hwnd, SB_VERT,
      (LPINT) & OwnVerMin, (LPINT) & OwnVerMax);
  GetScrollRange( hwnd, SB_HORZ,
      (LPINT) & OwnHorMin, (LPINT) & OwnHorMax);
...
void PASCAL RestoreOwnerScrollInfo(register HWND   hwnd)
...
void PASCAL InitOwnerScrollInfo()
...
if ((cCBCount = VCountClipboardFormats( GETMDIINFO(hwndMDI)->pVClpbrd ))
   ...
   for (fmt=0, cFmt=1; cFmt <= cCBCount; cFmt++)
      ...
      if ( fmt != cf_preview )
         {
         GetClipboardName(fmt, szName, CharSizeOf(szName));
...
BOOL PASCAL ClearClipboard(register HWND   hwnd)
...
if ( MessageBoxID( hInst, hwnd, IDS_CONFIRMCLEAR, IDS_CLEARTITLE,
   ...
   if (RetVal = SyncOpenClipboard(hwnd))
      ...
      {
      PERROR(TEXT("ClearClipboard: could not open\r\n"));
      }

   if (!RetVal)
      {
      MessageBoxID ( hInst, hwnd, IDS_CLEAR, IDS_ERROR,
