...
HISTORY
    ...
    sds		6 Dec 88	Initial Coding
		...
		25 Mar 2000	LoadResource assertion does better error
		29 Mar 2000	fixed bug in ForceTrActivat'ion
				better shutdown handling
		30 Mar 2000	LdStr now takes parent hWnd arg
		6 Apr 2000	now does GETMINMAXINFO, break dlg box on
				any form of record stop
		26 Apr 2000	changed stop-record on activate handling
		27 Apr 2000	now does break on sysmodal dlg in record mode
		2 May 2000	DoDlg now handles dlgbox failure (NOMEM)
		3 May 2000	absolutely positively refuses to restore
				window if still recording
		9 May 2000	KeyWndProc
		11 May 2000	WIN2 fix
		16 May 2000	tdyn.h now tracerd.h
		18 May 2000	added REMOTECTL stuff
		18 May 2000	massive user intfc changes
		22 May 2000	ID_CMDLINE support
		23 May 2000	now releasing code segment after playback
		25 May 2000	now does dir change on cmd from other tracer
		26 May 2000	WIN2 upgrade
		7 Jun 2000	changed macro delete handling
		9 Jun 2000	name change to recorder
    sds		12 Jun 2000	bProtMode now bNotBanked
    sds		30 Jun 2000	fixed listbox index bug in DeleteMacro
				Merge option now permanently on.  OK
				  to merge into blank "new" file.
    sds		3 Jul 2000	now using wsprintf
    sds		27 Jul 2000	no longer using toupper
				.hlp loaded from .RC
    sds		7 Aug 2000	PeekABoo now public
    sds		9 Aug 2000	Delete of non-hotkey macro now displays
				  comment in confirmation msgbox
    sds		29 Aug 2000	HELP menu changes
    sds		18 Sep 2000	fixed bug losing recording on exit from Win
    sds		20 Sep 2000	WM_MINPLAYBACK
    sds		21 Sep 2000	better handling of pb error during recording
    sds		29 Sep 2000	resizing out gap under listbox on WM_SIZE
    sds		10 Oct 2000	listbox now NOINTEGRALHEIGHT, removed unused
				  code
				yet another change to WinHelp calls
   				cmdline from new instance now better about
				  saving existing data
    sds		16 Oct 2000	delete of last macro now retains active
				  file.  File/Save deletes it.
    sds		30 Oct 2000	fixed nested hotkey disable problem
    sds		31 Oct 2000	FreeMacro change, added WinHelp(QUIT)
				WM_POSTERROR now calls ErrMsgArg
    sds		22 Nov 2000	workaround for listbox scrolling first
				macro out of site on File/Load Recorder x.rec
    sds		30 Nov 2000	changed MB_ICONQUESTION to MB_ICONEXCLAMATION
    sds		4 Dec 2000	WIN2 compatibility fix
    sds		6 Dec 2000	accelerator handling
    ...
    sds 	7 Jan 2000	fcn def changes for MSC6
