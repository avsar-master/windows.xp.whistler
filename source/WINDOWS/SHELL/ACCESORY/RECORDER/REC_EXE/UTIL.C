...
HISTORY
    ...
    sds		30 Nov 88	Initial Coding
		...
		29 Mar 2000	prettier errormsg for cmd line stuff
		30 Mar 2000	preloaded M_ERROR,M_WARNING
		2 May 2000	now checking LockResource ret code
		9 May 2000	just removed some unused #ifdef stuff
		16 May 2000	tdyn.h now tracerd.h
		18 May 2000	massive user intfc changes
    sds		23 May 2000	no longer releases code/desc if file changed
    sds		9 Jun 2000	name change to recorder
				fixed bug in GetKeyCode string matching
				ena/disable LB to prevent focus if empty
    sds		16 Jun 2000	now turns off redraw when loading listbox
    sds		3 Jul 2000	now using wsprintf
    sds		27 Jul 2000	AnsiUpper, not toupper
				ifdef'ed out local heap debugging
				totally new keycode/name resource handling
    sds		28 Jul 2000	Yet another keycode/name resource handler,
				since Win2 and Win3 RC have different
				sets of problems generating '\0' data bytes
    sds		17 Aug 2000	AddMacro now sets playback id
    sds		10 Oct 2000	LoadMacroList now enables listbox
    sds		31 Oct 2000	FreeMacro now takes bDelHotKey arg
    sds		26 Dec 2000	CheckComment
    sds		8 Feb 1990	now set focus to hWndRecorder if LB empty
    				Win2.1 compatibility
				now disallows unmappable keys as shortcuts
    sds 	7 Jan 2000	cleanups for MSC6 W4
*/
/* -------------------------------------------------------------------- */
