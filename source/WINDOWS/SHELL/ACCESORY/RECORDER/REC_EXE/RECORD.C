...
HISTORY
    ...
    sds		3 Jan 89	now uses global szWinOldApp
		...
		30 Mar 2000	new LdStr with hWnd
		4 Apr 2000	bug fix - wasn't always clearing pmNew
		5 Apr 2000	now using RecordBreak for ALL recording
		6 Apr 2000	record suspend/Resume changed
		26 Apr 2000	JPActivate now can restore too
		2 May 2000	DoDlg now handles dlgbox failure (NOMEM)
		13 May 2000	now releases macro code segment more often
		16 May 2000	takes advantage of protectmode to not
				use GMEM_NOT_BANKED memory
		19 May 2000	PlayHotKey now returns success
		9 Jun 2000	name change to recorder
    sds		12 Jun 2000	bProtMode now bNotBanked, bNoFlash
    sds		7 Aug 2000	bug fix for ctrl/numlock == pause
				now doing PeekABoo and retry on JPActivate
				failure
    sds		10 Aug 2000	JPActivate now loops and PeekABoo's for up
				to as long as record-time msg interval
    sds		17 Aug 2000	playback id now set in AddMacro, passed
				to AddHotKey
    sds		18 Sep 2000	RecordStop now sets bSuspended
    sds		20 Sep 2000	now smarter about iconizing on playback,
				especially if in menu mode
    sds		31 Oct 2000	FreeMacro now takes extra arg
    sds		28 Nov 2000	no longer clicks on desktop to bring menus
    ...
    sds 	7 Jan 2000	cleanups for MSC6
