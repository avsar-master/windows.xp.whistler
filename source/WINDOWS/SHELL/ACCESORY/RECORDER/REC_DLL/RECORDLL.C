...
HISTORY
    ...
    sds		16 Mar 89	hWndTarget at PB now ignores capture for
				  key msgs
    sds		27 Mar 2000	now handling HC_SYSMODALx
				bugfix where Heuristics deletes everything
    sds		30 Mar 2000	added workaround for Windows bug in trashing
				prevkeystate bit on KeyHook(HC_NOREM)
    sds		3 Apr 2000	[SYS]KEYUP's now playback to ANYBODY
    sds		4 Apr 2000	improved KEYUP playback
    sds		6 Apr 2000	RecordEvent now private
    sds		26 Apr 2000	added B_FORCEACTIVE bit to buttondown
				msg following junked alt/tab/esc sequence
    sds		27 Apr 2000	sysmodal dlgbox on record now suspends
				szDesktop is now #32769
				now using windows lstrcmp
    sds		3 May 2000	win2 support fix
    sds		5 May 2000	now using GetDesktopHwnd
    sds		9 May 2000	changed check for msg to Tracer
    sds		13 May 2000	fixed bug in recording event timing
    sds		16 May 2000	no NOT_BANKED memory in protect mode
				renamed from tdyn.c
    sds		22 May 2000	pb mouseUP to overlapped window now OK
    sds		23 May 2000	now notifies Tracer on pb done
    sds		9 Jun 2000	name change to recorder
    sds		12 Jun 2000	bProtMode now bNotBanked
    sds		28 Jul 2000	BreakPlayback for REMOTECTL
    sds		7 Aug 2000	increased MAXWAITPAINT, sometimes they really
				wail on JPHook without painting anything
    sds		8 Aug 2000	now passing event ptr on WM_JPACTIVATE, for
				  expanded wait-for-window-to-appear
    sds		17 Aug 2000	bug fix - now detecting nested pb via
				 id, not unreliable hMem
				now allows save of macro on TOOLONG error
    sds		25 Aug 2000	removed "include winexp.h", in windows.h now
    sds		19 Sep 2000	now using PostMessage(LBUTTONDOWN) to
				desktop to clear menu mode
    sds		20 Sep 2000	WIN2 stuff, winexp.h needed
    sds		10 Oct 2000	bug fix handling no-break check option
    sds		27 Oct 2000	now handles bizarre case of finding
				  playback target window, yet it is
				  destroyed before next JPHook call
    sds		31 Oct 2000	fixed innocuous bug in AddString
    sds		26 Dec 2000	no longer produces PB errors for mouse MOVE
    ...
    sds		8 Jan 2000	fixes for MSC6, fixed bug with combobox's
