...

{
    ...
    StartBenchMark("Path Conversion Test (e:\\xp\\windows\\foo)",
                   PATH_CONVERSION_TEST,
                   &PerfInfo);
...
Ofprompt()
{
    ...
    h = OpenFile("e:\\xp\\xt.cfg",&of,OF_PROMPT);
    printf("OpenFile(e:\\xp\\xt.cfg) h = %lx, GLE = %d\n",h,GetLastError());

    h = OpenFile("e:\\zznt\\xt.cfg",&of,OF_PROMPT);
    printf("OpenFile(e:\\zznt\\xt.cfg) h = %lx, GLE = %d\n",h,GetLastError());

    h = OpenFile("e:\\xp\\xt.cfg",&of,OF_PROMPT | OF_CANCEL);
    printf("OpenFile(e:\\xp\\xt.cfg) h = %lx, GLE = %d\n",h,GetLastError());
    SetErrorMode(0);
    h = OpenFile("e:\\xp\\xt.cfg",&of,OF_PROMPT);
    printf("OpenFile(e:\\xp\\xt.cfg) h = %lx, GLE = %d\n",h,GetLastError());
